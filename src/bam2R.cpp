...
void bam2R(char **bamfile, char **ref, int *beg, int *end, int *counts, int *q,
           int *mq, int *s, int *head_clip, int *maxdepth, int *verbose,
  ...
  if (strcmp(*ref, "") == 0) { // if a region is not specified
    ...
    while ((ret = sam_read1(nttable.in, head, b)) >= 0) {
      if (((b->core.flag & *mask) == 0 && b->core.qual >= *mq) &&  // {  // as 1.27.1 if this line only
